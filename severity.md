...
| Unclear physical role or definition choice (principle 11) | `SUGGEST`; for possible scientific ambiguity, `ESCALATE_TO: math` (or narrative if no math verifier) for class 6 assessment. Do not invent meaning. |
| Shifted field-standard meaning (principle 15) | `SUGGEST` + `ESCALATE_TO: math` (or narrative if no math verifier); do not silent-rename or silently broaden the term |
